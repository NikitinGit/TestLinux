...
public interface UserRepository extends CrudRepository<User, Integer> {
    ...
    Optional<User> findUserById(Integer login);
...
/*    @Query(value = "SELECT u.id AS user_id, u.pass, u.telefon, " +
    ...
    //@Query("SELECT u FROM User u JOIN FETCH u.fighter JOIN FETCH u.judge JOIN FETCH u.trainer WHERE u.id = :id")
    ...
    @Query("SELECT new com.example.testlinux.dto.UserEntityDto(u.fighter) FROM User u " +
            "LEFT JOIN u.fighter f " +
            "WHERE u.id = :id")
    Optional<UserEntityDto> getRole(Integer id);

    @Query("SELECT new com.example.testlinux.dto.UserEntityFighterDto(u.fighter) FROM User u " +
            "LEFT JOIN u.fighter f " +
            "WHERE u.id = :id")
    ...
    @Query("SELECT t FROM User t WHERE t.pass = :acstatus AND t.telefon = :telefon")
    Optional<User> userExists(String acstatus, String telefon);
}
